...
class ReportForge:
    ...
    async def generate(
        ...
        try:
            path = self._generate_pdf(
                pdf_path, config, retrieval_scores,
                adversarial_report, ragas_result, beir_result, extra_metrics
            )
            ...
            path = self._generate_markdown(
                ...
                config, retrieval_scores, adversarial_report, ragas_result
            )
    ...
    def _generate_pdf(
        ...
        h1_style = ParagraphStyle(
            ...
            textColor=colors.HexColor("#0A1230"), spaceBefore=20, spaceAfter=8,
        )
        h2_style = ParagraphStyle(
            "H2", parent=styles["Heading2"],
            fontSize=13, fontName="Helvetica-Bold",
            textColor=colors.HexColor("#1a3a6b"), spaceBefore=14, spaceAfter=6,
        )
        body_style = ParagraphStyle(
            "Body", parent=styles["Normal"],
            fontSize=10, fontName="Helvetica",
            textColor=colors.HexColor("#1e1e3a"), spaceAfter=4, leading=15,
        )
        metric_style = ParagraphStyle(
            "Metric", parent=styles["Normal"],
            fontSize=22, fontName="Helvetica-Bold",
            textColor=colors.HexColor("#00CCFF"), alignment=TA_CENTER,
        )
        ...
        if retrieval_scores:
            summary_data = [
                ...
                ["NDCG@10",      f"{retrieval_scores.ndcg_at_10:.4f}", self._interpret_ndcg(retrieval_scores.ndcg_at_10)],
                ["MRR",          f"{retrieval_scores.mrr:.4f}",         self._interpret_mrr(retrieval_scores.mrr)],
                ["MAP@10",       f"{retrieval_scores.map_at_10:.4f}",   "Average Precision across all queries"],
                ["Precision@10", f"{retrieval_scores.precision_at_10:.4f}", "Fraction of top-10 that are relevant"],
                ["Recall@10",    f"{retrieval_scores.recall_at_10:.4f}", "Fraction of relevant docs retrieved"],
                ["Hit Rate@10",  f"{retrieval_scores.hit_rate_at_10:.4f}", "Queries with ≥1 relevant in top-10"],
            ...
            tbl.setStyle(TableStyle([
                ...
                ("FONTSIZE",    (0,1), (-1,-1), 10),
                ("ROWBACKGROUNDS", (0,1), (-1,-1), [colors.HexColor("#f4f7ff"), colors.white]),
        ...
        if adversarial_report:
            ...
            tbl = Table(adv_data, colWidths=[2.2*inch, 1.3*inch, 1.3*inch, 0.8*inch, 1.2*inch])
            tbl.setStyle(TableStyle([
    ...
    def _generate_markdown(
        ...
        if adversarial:
            lines += ["", "## Adversarial Robustness", f"**Overall: {adversarial.overall_robustness:.1f}%**", ""]
            for ar in adversarial.attack_results:
                lines.append(f"- {ar.attack_name}: {ar.attacked_ndcg:.4f} (Δ{ar.degradation_pct:+.1f}%)")

        path.write_text("\n".join(lines))
        return path

    def _interpret_ndcg(self, score: float) -> str:
        if score >= 0.85: return "Excellent — production-ready"
        if score >= 0.70: return "Good — competitive performance"
        if score >= 0.55: return "Moderate — improvement needed"
        return "Poor — investigate retrieval quality"

    def _interpret_mrr(self, score: float) -> str:
        if score >= 0.90: return "Excellent — first result almost always relevant"
        if score >= 0.75: return "Good — relevant result typically in top 2"
        if score >= 0.60: return "Moderate — often not in position 1"
    ...
    def _generate_recommendations(self, scores, adv, ragas) -> list[str]:
        ...
        if scores:
            if scores.ndcg_at_10 < 0.70:
                recs.append("Switch to hybrid retrieval (RRF fusion) if using sparse-only — typically +8-12% NDCG@10.")
            if scores.recall_at_10 < 0.60:
                recs.append("Improve recall: increase top_k to 20, add query expansion, or use dense retrieval with BGE-M3.")
            if scores.precision_at_10 < 0.50:
                recs.append("Improve precision: add cross-encoder reranking (RankForge) — eliminates low-relevance results.")
            if scores.mrr < 0.70:
                recs.append("Improve MRR: PropositionalChunker produces finer granularity for factoid Q&A; test on your query set.")

        if adv and adv.overall_robustness < 80:
            recs.append("Adversarial robustness < 80%: add query augmentation in training, use spell-correction as a preprocessing step.")

        if ragas and ragas.faithfulness and ragas.faithfulness < 0.80:
            recs.append("Low faithfulness score: the LLM is hallucinating. Add explicit grounding instructions and reduce temperature to 0.")

        if not recs:
            recs.append("All metrics within target range. Consider running BEIR benchmark for external validation.")
            recs.append("Next: fine-tune embedding model with domain-specific data using LoRA for +5-10% NDCG improvement.")
